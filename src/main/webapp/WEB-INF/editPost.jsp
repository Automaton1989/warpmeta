	...
	<div class="container-fluid" id="createPost">
	    <div class="row">
		    <div class="offset-2 col-8">
		        <form:form id="createPostForm" modelAttribute='post' action="/edit-post/${post.id}/edit" method="put" enctype="multipart/form-data">
		       	<input type="hidden" name="_method" value="put">
		            <fieldset>
		                <legend>Add a post</legend>
		                <div class="form-group">
		                    <label for="title">Post Title: </label>
		                    <form:input id="title" path="title" class="form-control" value="${post.title}" />
		                    <form:errors path="title"/>
		                </div>
		               	<div class="form-group">
		                    <label for="description">Short description: </label>
		                    <form:input id="description" path="description" class="form-control" value="${post.description}" />
		                </div>
		               	<div class="form-group">
		                    <label for="content">Content: </label>
		                    <form:textarea id="NicEdit" path="content" rows="5" class="form-control" value="${post.content}"/>
		                </div>
		                	<form:input type="hidden" path="creator" value="${user.id}" />
		                <div>		                    
		                <button type="submit" tabindex="5" class="btn">Edit Post!</button>
		                </div>
		            </fieldset>
		        </form:form>
		    </div>
